...
async function api(path, params, cfg) {
...
async function getChannels(cfg) {
  return api('/tv/v2/channels', { ...commonParams(cfg), 'tv-asset-token': cfg.tvAssetToken }, cfg);
}

async function getMedias(cfg) {
  return api('/tv/v2/medias', { ...commonParams(cfg), 'tv-asset-token': cfg.tvAssetToken }, cfg);
}

async function getPlayback(mediaId, cfg) {
  const enable50fps = cfg.enable50fps ? 'true' : 'false';
  return api(`/playback-info-media/${mediaId}`, { ...commonParams(cfg), enable50fps }, cfg);
}
...
function pickStreamUrl(playback, quality = 'Auto') {
  ...
  if (quality === 'Auto') {
    ...
    if (!chosen) {
      ...
      if (!chosen) chosen = rends.find(r => r.id === 'Auto' || r.default) || rends[0];
    }
  }
...
async function main() {
  ...
  for (const m of medias) {
    if (!m.isLocked) mediaByChannel.set(m.channelId, m);
  }
  ...
  await mapLimit(jobs, cfg.concurrency, async (job) => {
    try {
      ...
      if (!picked) throw new Error('no stream URL in response');
      entries.push({ ...job, url: picked.url, quality: picked.quality, pbsId: playback.pbsId, expires: playback.expires });
    } catch (err) {
  ...
  const m3uPath = `${OUT_DIR}/telecomtv.m3u`;
